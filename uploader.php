...
// Get the device configuration file (xochitl.conf)
function getDeviceConfig(): array {
    // Check connection first
    $connection = checkConnection();
    if (!$connection['success']) {
        return ['success' => false, 'message' => 'Not connected to reMarkable: ' . $connection['message']];
    ...
    // Create temp file
    $tmpFile = tempnam(sys_get_temp_dir(), 'remarkable_conf_');
    
    // Download config file
    $download = downloadFile('/home/root/.config/remarkable/xochitl.conf', $tmpFile);
    
    if (!$download['success']) {
        // Check if file doesn't exist
        if (strpos($download['message'], 'No such file') !== false) {
            // Create an empty default config for devices that don't have one yet
            return [
                'success' => true, 
                'config' => "# reMarkable Configuration File\n# Generated by reMarkable Template Manager\n\n# This file was newly created as no configuration existed.\n# Add configuration parameters below in format: key=value\n"
            ];
        }
        
        return ['success' => false, 'message' => 'Failed to download configuration: ' . $download['message']];
    }
    
    // Read the config file
    $configContent = file_get_contents($tmpFile);
...
function saveDeviceConfig(string $config): array {
    // Check connection first
    $connection = checkConnection();
    if (!$connection['success']) {
        return ['success' => false, 'message' => 'Not connected to reMarkable: ' . $connection['message']];
    ...
    if (empty($config)) {
        return ['success' => false, 'message' => 'Configuration cannot be empty'];
    }
    ...
    // Upload config file
    $upload = uploadFile($tmpFile, '/home/root/.config/remarkable/xochitl.conf');
    
    // Clean up
    unlink($tmpFile);
    
    if (!$upload['success']) {
        return ['success' => false, 'message' => 'Failed to upload configuration: ' . $upload['message']];
    }
    
    // Set correct permissions
    $chmodCmd = "chmod 644 /home/root/.config/remarkable/xochitl.conf";
    $chmodResult = executeSSHCommand($chmodCmd);
    ...
    // Restart the device UI service to apply changes
    $restart = restartRemarkable();
...
case 'get_device_config':
    echo json_encode(getDeviceConfig());
    break;
case 'save_device_config':
    ...
    echo json_encode(saveDeviceConfig($config));
    break;
        default:
